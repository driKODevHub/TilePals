...
namespace CodeMonkey.Utils
{
    ...
    public static class UtilsClass
    {
        ...
        {
            // Uses the primary CreateWorldText with default scale (Vector3.one)
            TextMesh textMesh = CreateWorldText(parent, GetTextFunc(), localPosition, Vector3.one * 0.05f, 40, Color.white, TextAnchor.UpperLeft, TextAlignment.Left, sortingOrderDefault);
        ...
        // Create Text in the World (Convenience overload with default scale of Vector3.one)
        // If you don't specify localScale, it will default to 1,1,1
        public static TextMesh CreateWorldText(string text, Transform parent = null, Vector3 localPosition = default(Vector3), int fontSize = 40, Color? color = null, TextAnchor textAnchor = TextAnchor.UpperLeft, TextAlignment textAlignment = TextAlignment.Left, int sortingOrder = sortingOrderDefault)
        {
            if (color == null) color = Color.white;
            return CreateWorldText(parent, text, localPosition, Vector3.one, fontSize, (Color)color, textAnchor, textAlignment, sortingOrder);
        }

        // Create Text in the World (Primary method, accepts localScale)
        public static TextMesh CreateWorldText(Transform parent, string text, Vector3 localPosition, Vector3 localScale, int fontSize, Color color, TextAnchor textAnchor, TextAlignment textAlignment, int sortingOrder)
        {
            GameObject gameObject = new GameObject("World_Text", typeof(TextMesh));
            Transform transform = gameObject.transform;
            transform.SetParent(parent, false);
            transform.localPosition = localPosition;
            transform.localScale = localScale; // This is where the scale is applied
        ...
        {
            // Uses the primary CreateWorldText with default scale (Vector3.one) for popups
            TextMesh textMesh = CreateWorldText(parent, text, localPosition, Vector3.one, fontSize, color, TextAnchor.LowerLeft, TextAlignment.Left, sortingOrderDefault);
